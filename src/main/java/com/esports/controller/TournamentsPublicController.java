...
public class TournamentsPublicController implements Initializable {
    ...
    private String formatPrice(double priceInEUR) {
        ...
        switch (currentCurrency) {
            case TND: converted = priceInEUR * RATE_TND; symbol = " DT"; break;
            case USD: converted = priceInEUR * RATE_USD; symbol = " $"; break;
        }
    ...
    private void onOpenGlobalMap() {
        ...
        if (allTournaments == null || allTournaments.isEmpty()) allTournaments = dao.findAll();
        ...
        infoBox.setVisible(false); infoBox.setManaged(false);
        infoBox.setPadding(new Insets(20)); infoBox.setMaxWidth(320);
        ...
        Label infoTitle = new Label(); infoTitle.setWrapText(true);
        infoTitle.setStyle("-fx-text-fill: white; -fx-font-weight: bold; -fx-font-size: 15px;");
        Label infoDetails = new Label(); infoDetails.setWrapText(true);
        infoDetails.setStyle("-fx-text-fill: #9ca3af; -fx-font-size: 13px; -fx-line-spacing: 4px;");
        Button btnAction = new Button(); btnAction.setMaxWidth(Double.MAX_VALUE);
        ...
        mapView.addEventHandler(com.sothawo.mapjfx.event.MapViewEvent.MAP_CLICKED, event -> {
            ...
            if (coord != null && fPickpoint != null) {
                fPickpoint.setPosition(coord); fPickpoint.setVisible(true);
                ...
                btnAction.setOnAction(e -> {
                    double pLat = coord.getLatitude(), pLon = coord.getLongitude();
                    List<Tournament> nearby = allTournaments.stream().filter(t -> {
                        ...
                        try {
                            ...
                            return calculateDistance(pLat, pLon, tLat, tLon) <= 100;
                        } catch (Exception ex) { return false; }
                    }).toList();
                    if (nearby.isEmpty()) showAlert("NexUS Discovery", "Aucun tournoi n'a été trouvé dans un rayon de 100km.");
                    else { stage.close(); displayTournaments(nearby); showAlert("NexUS Discovery", nearby.size() + " tournois trouvés à proximité !"); }
                });
                infoBox.setVisible(true); infoBox.setManaged(true);
            }
        });
    ...
    private void displayTournaments(List<Tournament> list) {
        paneOpen.getChildren().clear(); paneUpcoming.getChildren().clear();
        paneInProgress.getChildren().clear(); paneFinished.getChildren().clear();
        lblTotalCount.setText(String.valueOf(list.size()));
        for (Tournament t : list) {
            VBox card = buildCard(t);
        ...
        lblEmpty.setVisible(list.isEmpty()); lblEmpty.setManaged(list.isEmpty());
    }
    ...
    private void addMarker(MapView mapView, String location, Marker.Provided color, Tournament data, Map<String, Tournament> markerMap) {
        ...
        if (parts.length >= 2) {
            try {
                ...
                Marker marker = new Marker(markerUrl, -12, -41).setPosition(new Coordinate(lat, lon)).setVisible(true);
                mapView.addMarker(marker);
                markerMap.put(marker.getId(), data);
            } catch (Exception e) { System.err.println("[MAP] Erreur format : " + location); }
        }
    }
    ...
    private void onSendMessage() {
        ...
        if (localResponse != null) { chatArea.appendText(localResponse + "\n\n"); return; }
        ...
        new Thread(() -> {
            ...
            Platform.runLater(() -> {
                ...
                if (lastIdx != -1) chatArea.setText(currentText.substring(0, lastIdx));
    ...
    private String askGemini(String prompt) {
        try {
            ...
            String jsonBody = new JSONObject().put("contents", new JSONArray().put(new JSONObject()
                .put("parts", new JSONArray().put(new JSONObject().put("text", systemPrompt))))).toString();
            HttpRequest request = HttpRequest.newBuilder().uri(URI.create(GEMINI_API_URL + GEMINI_API_KEY))
        ...
        } catch (Exception e) { return "Une erreur est survenue lors de la communication avec l'IA."; }
    }
    ...
    private void onStartLive() {
        ...
        if (result.isPresent()) {
            ...
            if (!twitchPseudo.isEmpty()) showNexUSStudio(twitchPseudo);
        }
    }
    ...
    private void showNexUSStudio(String pseudo) {
        ...
        screenPreview.setFitWidth(640); screenPreview.setFitHeight(360); screenPreview.setPreserveRatio(true);

        WebView webcamView = new WebView();
        webcamView.setPrefSize(160, 120); webcamView.setMaxSize(160, 120);
        ...
        ProgressBar micLevel = new ProgressBar(0); micLevel.setPrefWidth(640);
        ...
        try {
            ...
            btnStart.setOnAction(e -> {
                try {
                    line.open(format); line.start(); timer.start();
            ...
            btnStop.setOnAction(e -> {
                timer.stop(); line.stop(); line.close();
                btnStart.setDisable(false); btnStop.setDisable(true);
                screenPreview.setImage(null); micLevel.setProgress(0);
            });

            HBox controls = new HBox(20, btnStart, btnStop); controls.setAlignment(Pos.CENTER);

            String twitchUrl = "https://www.twitch.tv/" + pseudo;
            Label lblShare = new Label("PARTAGER MON LIVE :"); lblShare.setStyle("-fx-text-fill: #9ca3af; -fx-font-size: 11px; -fx-font-weight: bold;");
            Button btnFb = new Button("Facebook"); btnFb.setStyle("-fx-background-color: #1877f2; -fx-text-fill: white; -fx-cursor: hand;");
            btnFb.setOnAction(e -> share("https://www.facebook.com/sharer/sharer.php?u=" + twitchUrl));
            Button btnX = new Button("Twitter (X)"); btnX.setStyle("-fx-background-color: #000000; -fx-text-fill: white; -fx-border-color: #4b5563; -fx-cursor: hand;");
            btnX.setOnAction(e -> share("https://twitter.com/intent/tweet?text=Je suis en direct sur NexUS !&url=" + twitchUrl));
            Button btnWa = new Button("WhatsApp"); btnWa.setStyle("-fx-background-color: #25d366; -fx-text-fill: white; -fx-cursor: hand;");
            btnWa.setOnAction(e -> share("https://api.whatsapp.com/send?text=Venez voir mon live NexUS ! " + twitchUrl));
            Button btnCopy = new Button("📋 Copier le lien"); btnCopy.setStyle("-fx-background-color: #4b5563; -fx-text-fill: white; -fx-cursor: hand;");
            btnCopy.setOnAction(e -> {
                Clipboard clipboard = Clipboard.getSystemClipboard();
                ClipboardContent content = new ClipboardContent();
                content.putString(twitchUrl); clipboard.setContent(content);
                btnCopy.setText("✅ Copié !");
            });

            HBox shareBar = new HBox(10, lblShare, btnFb, btnX, btnWa, btnCopy);
            shareBar.setAlignment(Pos.CENTER); shareBar.setPadding(new Insets(10, 0, 0, 0));

            root.getChildren().addAll(title, mediaPane, micBox, controls, shareBar);
            Scene scene = new Scene(root, 900, 700);
            studio.setScene(scene); studio.show();
            studio.setOnCloseRequest(e -> { timer.stop(); if (line.isOpen()) line.close(); });
        } catch (Exception ex) {
    ...
    private String accentColor(String s) {
        return switch (s) {
            case "OUVERT" -> "#4ade80"; case "DRAFT" -> "#00b8ff";
            case "EN_COURS" -> "#ec4899"; default -> "#4b5563";
        };
    }

    private String badgeText(String s) {
        return switch (s) {
            case "OUVERT" -> "● OUVERT"; case "DRAFT" -> "◆ À VENIR";
            case "EN_COURS" -> "▶ EN COURS"; case "TERMINE" -> "✓ TERMINÉ"; default -> s;
        };
    }
